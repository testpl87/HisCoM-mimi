...
perform_cv=function(orig_dat,y,group_info,lambda_list=seq(0.05,0.5,by=0.05),err_rate=0.0001,iter_num=1000,cvnum=5){

	num_samp_all=nrow(orig_dat)
	...
	ytrain=y[train_dat]
	xtrain=orig_dat[train_dat,]
	ytest=y[test_dat]
	xtest=orig_dat[test_dat,]

	result_inter=perform_hiscom(xtrain,ytrain,group_info,lambda,lambda,iter_num=iter_num)
